 ...
 *     time  : 2019/06/20
 *     desc  : SlcUtils about language
 * </pre>
 */
public class SlcLanguageUtils {

    private static final String KEY_LOCALE          = "KEY_LOCALE";
    private static final String VALUE_FOLLOW_SYSTEM = "VALUE_FOLLOW_SYSTEM";

    private SlcLanguageUtils() {
    ...
    private static void applyLanguage(@NonNull final Locale locale,
                                      ...
                                      final boolean isNeedStartActivity) {
        if (isFollowSystem) {
            SlcUtils.getSlcSPUtils4Utils().put(KEY_LOCALE, VALUE_FOLLOW_SYSTEM);
        } else {
            String localLanguage = locale.getLanguage();
            String localCountry = locale.getCountry();
            SlcUtils.getSlcSPUtils4Utils().put(KEY_LOCALE, localLanguage + "$" + localCountry);
        }

        updateLanguage(SlcUtils.getApp(), locale);
        ...
        if (isNeedStartActivity) {
            ...
            intent.setComponent(new ComponentName(SlcUtils.getApp(), realActivityClassName));
    ...
    /**
     * Return whether applied the system language by {@link SlcLanguageUtils}.
     *
     * @return {@code true}: yes<br>{@code false}: no
     */
    public static boolean isAppliedSystemLanguage() {
        return VALUE_FOLLOW_SYSTEM.equals(SlcUtils.getSlcSPUtils4Utils().getString(KEY_LOCALE));
    }

    /**
     * Return whether applied the language by {@link SlcLanguageUtils}.
     *
     * @return {@code true}: yes<br>{@code false}: no
     */
    public static boolean isAppliedLanguage() {
        return !TextUtils.isEmpty(SlcUtils.getSlcSPUtils4Utils().getString(KEY_LOCALE));
    }
    ...
    public static Locale getCurrentLocale() {
        return SlcUtils.getApp().getResources().getConfiguration().locale;
    }

    static void applyLanguage(@NonNull final Activity activity) {
        final String spLocale = SlcUtils.getSlcSPUtils4Utils().getString(KEY_LOCALE);
        ...
        if (VALUE_FOLLOW_SYSTEM.equals(spLocale)) {
            ...
            updateLanguage(SlcUtils.getApp(), sysLocale);
        ...
        updateLanguage(SlcUtils.getApp(), settingLocale);
    ...
    private static String getLauncherActivity() {
        ...
        intent.setPackage(SlcUtils.getApp().getPackageName());
